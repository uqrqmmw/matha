...
test('跨裝置人工覆核不同題時逐題合併，且保留詳批解鎖時間', () => {
  ...
  const result = plain(run(`(() => {
    ...
    const a = { paperRuns:[{ id:'p1', mt:100, createdAt:1,
      ...
      review:{ 1:{ attempts:1, solutionUnlockedAt:100, detailFirstOpenedAt:100, detailLastOpenedAt:120, detailViewCount:1, logs:[{ ts:100, note:'A' }] } },
    }] };
    const b = { paperRuns:[{ id:'p1', mt:200, createdAt:1,
      ...
      review:{ 1:{ attempts:1, detailFirstOpenedAt:150, detailLastOpenedAt:220, detailViewCount:2, logs:[{ ts:200, note:'B' }] } },
    }] };
    return mergeState(a, b).paperRuns[0];
  })()`));
  ...
  assert.equal(result.review['1'].detailFirstOpenedAt, 100);
  assert.equal(result.review['1'].detailLastOpenedAt, 220);
  assert.equal(result.review['1'].detailViewCount, 2);
