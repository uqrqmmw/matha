...
前六本之後,《平面向量與內積應用》《方程式與矩陣運算》也已完成 map 與裁切(584 頁、721 題)。矩陣書三章的 easy→medium→hard 分層零調參全對;向量書是**重度使用過的紙本**——鉛筆答案直接寫在題目的印刷空格裡、旁邊畫演算圖,151 題被實心墨判準正確送進人工巷道,這是紙的實況不是管線缺陷。

跨頁掛接嘗試了三版全數在目視驗證中失敗(這套書的分數式首行會向上碎裂,任何間隙門檻都會把**下一題**的碎片黏到前一題,甚至把一整題當成尾巴),已依證據**整個撤除**:頁頂無主內容一律進 `unattachedPageTops` 報告清單,列頁碼與範圍給人工,絕不自動黏。

## 目前的實跑結果（前六本）

| bookId | 頁 | 候選題 | 含圖題 | clean/repair | 漏題(有精確清單) |
|---|---:|---:|---:|---|---:|
| `matha-114-line-inequality` | 206 | 246 | 68 | 198/48 | 6 |
| `matha-114-polynomial-quadratic` | 246 | 330 | 61 | 247/83 | 15 |
| `matha-114-trig-graph` | 222 | 228 | 91 | 143/85 | 10 |
| `matha-114-sequence` | 262 | 329 | 104 | 209/120 | 4 |
| `matha-114-data` | 282 | 342 | 152 | 238/104 | 9 |
| `matha-114-space-plane-line` | 292 | 330 | 83 | 282/48 | 20 |

1,805 題全部產出裁切，0 題因越過答案邊界被拒絕；96% 的題有切好的答案圖。

題號被 OCR 讀丟的題走三層處理：頁內由印刷題號證明缺號且唯一候選才回收；跨頁缺口先在題號邊界找、再在寬邊界找被剪掉題號的殘行（前一題的續行以「垂直斷行」排除，`（1）` 子小題永不當候選）；仍回收不了的 64 題在各書 `qa-report.md` 留精確缺號清單（區塊、題型、缺哪幾號、哪幾頁）。曾試過放寬全頁掃描回收，多切出 559 個假題而一題真漏題都沒補回，該做法已廢棄——回收只在「印刷題號自己證明有缺」的地方發生。

## 已驗證與尚未驗證

已用實跑驗證：六本 1,510 頁全數索引；section map 的分章與 easy／medium／hard 分層逐段對照實體書無誤；題幹與圖形裁切目視抽查（題幹不含答案、座標軸與刻度完整、選項不進圖、單位圓標點齊全）；《數列遞迴與級數求和》是第一本完全沒有為它調參就跑出正確分章分層的書；鉛筆判準在乾淨的《直線與二元一次不等式》上數字完全不變（237 題／69 含圖），在有筆跡的頁上保留印刷圖、只標記鉛筆。`tests/test_ingest.py` 57 項與既有 `npm test` 215 項全數通過。
