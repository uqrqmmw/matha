...
def main() -> int:
    ...
    for source_id, (pdf_pages, count) in legacy.items():
        logical_pages = [(page, side) for page in pdf_pages for side in ("left", "right")]
        anchors = label_anchors(publisher, logical_pages)
        papers[source_id] = {
            "sourceSha256": publisher_sha,
            "questions": question_segments(anchors, count),
        }

    payload = {"schema": 1, "method": "embedded-pdf-question-anchors", "papers": papers}
