...
從 `0825o` 起，日常教材精選先收真正到期的錯題／2、7 日保留重測，再按目前能力證據混合打底、銜接、伸展三個訓練帶；冷啟動 10 題預設約 3／5／2，明顯斷裂時增加打底，穩定後才增加伸展。正常情況同單元、同一本教材各最多兩題；尚未完成安全 OCR 的數列、數據與空間等單元由 363 題核心庫補位，不再因已匯入十本教材題量大而消失。`vec3`／`space` 舊製作代碼也會在匯入邊界轉成正式的 `svec`／`splane`。
...
- `matha-content` bucket 是私有且僅允許登入者讀取。4521 題原始來源中，4138 題可安全出題；其中 76 題的必要圖形已通過來源雜湊、裁切像素與獨立複核。另有 374 題等待圖形 QA、6 題超出數A範圍、3 題與舊題重複，不會默默混入練習。
...
將輸出的 10 個內容雜湊單元包上傳到私有 `matha-content` bucket，並把前端 `PRIVATE_CONTENT_MANIFEST` 指定的版本化 manifest alias（目前為 `manifest-0825e.json`）同步上傳；不能只更新未被前端讀取的 `manifest.json`。通過雙人複核的裁圖要另從 promotion output 的 `figure-assets/` 依原相對路徑上傳至私有 `matha-figures` bucket。前端只在登入後下載，逐檔比對題包與圖檔 SHA-256；任一檔案不符就拒絕加入題庫，並退回 363 題離線核心庫。發版前必須在 Storage 實際確認 manifest alias、10 個 pack 與所有新增裁圖都存在，不能用本機 promotion 的 `uploadPerformed:false` 紀錄冒充已部署。
