...
整份施工藍圖另有 fail-closed 完工稽核；它會重新核對完整卷來源、真機 JSON、詳批 gold／prediction、透明擁有者授權題庫簽核與私有部署記錄，不會用測試綠燈替代尚未發生的真人證據：

```powershell
python scripts/audit-blueprint-readiness.py
```

結果固定輸出到桌面的 `數學系統完工稽核.md` 與 `.json`。報告明確分成 `engineeringComplete` 與 `capabilityValidated`：前者只在 6 道工程關卡全部通過時成立，後者另要求 5 道交付後真實使用關卡。相容欄位 `complete` 只代表工程交付，不會用假作答或測試資料補齊真機、個人 gold、新鮮卷與最近三回 72 分證據。
...
GitHub Actions 會在 push 與 pull request 自動執行專案檢查；最終仍須用 delivery verifier 證明目前乾淨 HEAD 的 CI、Pages 與線上 bytes，而不能只看舊 workflow 綠燈。
...
7. 正式私有題庫必須以同一 bundle 留下第一次部署 D1、綁定 D1 的回滾，以及時間較晚且記錄不同的最終部署 D2；不能用同一份部署記錄冒充兩次部署。
8. D2 後執行 `verify-private-release-runtime.py`，全量回讀 alias／版本化物件並核對簽核題源；這只證明 Storage，不代表 App 已能登入載入。
9. Storage 證據成立後執行 `verify-private-app-loader.py`，由啟用中的真實使用者 JWT 以 RLS 實載 191 題包與 217/217 題圖，再以 signed URL 做跨 14 單元／4 角色抽查；不得以 service role 下載代替。
10. 最後執行 `verify-github-delivery.py`；先掃描完整 tracked tree，拒絕 secret 與私有題圖／答案／bundle，再核對乾淨 `main` 等於 `origin/main`、同一 HEAD 的 CI／Pages 成功，且線上 `index.html`、`app.js`、`sw.js`、`textbook-catalog.js` 與本機逐 bytes 相同。
