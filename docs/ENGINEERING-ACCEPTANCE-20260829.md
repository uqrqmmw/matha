...
- App 快取版本為 `0901a`，正式卷協定版本另固定為 `0830b`；一般前端快取更新不會使既有或進行中的正式卷失效。

## 舒適排版與原題真值

- 原卷作答、首輪批改與隔日訂正共用同一個預設「舒適作答」模式：130% 顯示、題號前後導覽、自動把目前題定位在兩列浮動工具下方的安全區；使用者仍可一鍵切回整頁原卷。
- 排版只改閱讀尺度、留白與定位，不重寫題目內容；題面仍是已驗證的原始像素，因此不會因 OCR 重排新增中文、公式、負號、矩陣或圖形錯誤。
- 私人教材的逐題原 PDF 裁圖增加四周留白；原卷最後一段另保留底部捲動安全區，交卷與輸出按鈕不會讓末題永遠被遮住。
- 2026-09-01 以系統 Chrome 自動走過 1848×1104、1280×800、1024×600 三種橫向視窗；三者題目焦點均在工具列下方、控制列無水平溢出，舒適／整頁切換成功。此為工程像素 QA，不冒充 Galaxy Tab 的本人 100 分鐘真機證據。
...
- 2026-09-01 舒適排版版：Web／PWA 326 項（324 通過、2 項按設計跳過、0 失敗）；Python 383/383 通過；PostgreSQL 23 項（22 通過、1 項按設計跳過、0 失敗）；Supabase Edge 47/47 通過，Deno fmt／check 亦通過。
