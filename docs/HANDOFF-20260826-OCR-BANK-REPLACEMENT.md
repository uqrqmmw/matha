...
- 嚴格 QA 已掃描 6,720 頁：6,439 頁通過、281 頁列入複核；其中 3 頁確認為來源空白／封面，10 頁確認有來源內容但 OCR 漏掉。
- 10 頁已逐頁重跑 Mistral，但候選仍不能直接使用：抽查發現三列方程組漏掉整列，以及 `-5≤x<1` 被辨識成 `-5≤x≤1`。後者在舊 RapidOCR 也同樣讀錯，證明兩引擎一致不等於數學正確。
- QA 檔案：
