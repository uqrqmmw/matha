...
- 已核對 10 回完整來源：出版社模考 3 回、111–115 學測數學 A 正式卷 5 回、110 年試辦考試數學 A 1 回，以及 111 學年度起適用參考卷 1 回。
...
- 2026-08-29 已把 111–115 與 110 試辦轉成 48 張清晰完整單頁並上傳既有私有 `matha-papers` bucket；48/48 逐檔下載重算 SHA-256，與來源完全相符。另以 Supabase CLI 重新列出六個遠端目錄，建立 hash-bound `official-storage-readback-verification.json`，完工稽核會逐頁核對本機來源、App 引用、遠端名稱與回讀 bytes。六回都已接入 App 0829r，題號綁定 PDF 頁碼；正式答案與非選題公開評分原則已合併到交卷後 Edge secret，`openai-proxy` version 34。110 試辦的 8 頁完整官方詳解亦已私有上傳、回讀驗 hash，只有隔日重想後才簽發短效網址。仍未完成的是本人逐回新鮮度確認與 Galaxy Tab 真機驗收。
...
115 學測三份原檔已從舊診斷資料夾按上述 SHA-256 找回，並集中複製到私人素材根目錄的 `完整模考來源/115/`；110 試辦與 111 參考卷分別放在 `完整模考來源/110-trial/`、`完整模考來源/111-reference/`。`full-paper-inventory.json` 已加入可重現的 `pathHint`，完工稽核會逐檔重驗所有 24 份正式來源文件。
...
1. 在 Galaxy Tab S10 Ultra 對第三回執行既有七項開考檢查；本人確認未看過後，完成 100 分鐘作答、暫停／恢復、交卷與 PDF，再按「同步並匯出驗收檔」。合格證據由 Edge v34 重讀雲端狀態後自動封存到私人 Supabase，另下載本機備份。
2. 隔日完成不看詳解的重想，再驗證官方詳解與 AI 詳批閉環。
3. 111–115 與 110 試辦已接入；每回第一次建立 run 時 App 會詢問是否未看過。取消仍可練習，但該回不進級分校準；目前 7 回候選可支援第一階段 6 回，若不足再補外部新來源。
